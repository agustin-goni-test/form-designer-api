...
CREATE TABLE form_definition.components (
    id SERIAL PRIMARY KEY,
    key TEXT UNIQUE NOT NULL,  -- "EmailBox", "PhoneBox", "ToggleButton"
    name TEXT NOT NULL,        -- "Email Input Field"
    description TEXT,
    base_component TEXT,       -- Inherits from "TextBox", "Selection", etc.
    category TEXT NOT NULL,    -- "input", "choice", "layout", "custom"
    default_props JSONB,       -- Default properties
    validation_config JSONB,   -- Built-in validation rules
    service_bindings JSONB,    -- Default service endpoints
    created_at TIMESTAMPTZ DEFAULT now(),
    updated_at TIMESTAMPTZ DEFAULT now()
);

-- Table comments
COMMENT ON TABLE form_definition.components IS 'Base definitions for reusable form components (that inherit from standard components).';
...
COMMENT ON COLUMN form_definition.components.description IS 'Detailed description of the component';
COMMENT ON COLUMN form_definition.components.base_component IS 'Specifies the base component this component inherits from (e.g. "TextBox", "Selection")';
COMMENT ON COLUMN form_definition.components.category IS 'Category of the component (e.g. "input", "choice", "layout", "custom")';
COMMENT ON COLUMN form_definition.components.default_props IS 'Default properties for the component stored as JSON';
COMMENT ON COLUMN form_definition.components.validation_config IS 'Built-in validation rules for the component stored as JSON';
COMMENT ON COLUMN form_definition.components.service_bindings IS 'Default service endpoints for the component stored as JSON';
...
CREATE TABLE form_definition.component_versions (
    ...
    version_number INTEGER NOT NULL,
    definition JSONB NOT NULL,  -- Full component definition
...
-- Table comments
COMMENT ON TABLE form_definition.components IS 'Base definitions for reusable form components.';
...
COMMENT ON COLUMN form_definition.component_versions.definition IS 'Full component definition stored as JSON';
